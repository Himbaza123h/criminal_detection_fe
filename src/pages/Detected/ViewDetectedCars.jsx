...
import { fetchDetectedCars } from '../../slices/detectedSlice';

const ViewDetectedCars = () => {
  ...
  useEffect(() => {
    dispatch(fetchDetectedCars({ page: 1 }));
  }, [dispatch]);
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex flex-col flex-1 overflow-y-auto">
        ...
        <main className="flex-grow p-4 md:p-6 lg:p-8">
          <h1 className="text-3xl font-bold mb-6">Detected Cars</h1>
          {status === 'loading' ? (
            <p>Loading...</p>
          ) : detected.length === 0 ? (
            <p>No detected cars available</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {detected.map((detection) => (
                <div key={detection.id} className="bg-white p-6 rounded-lg shadow-md">
                  <h2 className="text-xl font-bold mb-2">{detection.suspect_car_plate}</h2>
                  
                  <p><strong>Detected Mode:</strong> {detection.detection_type}</p>
...
export default ViewDetectedCars;
